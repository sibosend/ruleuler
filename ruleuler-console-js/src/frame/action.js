...
/**
 * 纯函数：从 root 节点中提取指定 project 的 children，跳过 root 和 project 层级。
 * 返回一个与 rootNode 同结构的对象，children 替换为目标 project 的子节点。
 * 若目标 project 不存在或无 children，返回 children 为空数组的对象。
 */
export function extractProjectChildren(rootNode, projectName) {
    if(!rootNode || !rootNode.children) return {children: []};
    const project = rootNode.children.find(c => c.name === projectName);
    if(!project || !project.children) return {children: []};
    return Object.assign({}, rootNode, {children: project.children});
}

/**
 * 内部辅助：若处于单项目模式，对 rootFile 做树根提升；否则原样返回。
 */
function promoteTreeRoot(rootFile) {
    if(window._singleProjectMode && window._projectName){
        const promoted = extractProjectChildren(rootFile, window._projectName);
        promoted.name = window._projectName;
        return promoted;
    }
    return rootFile;
}
...
export function createNewFile(newFileName,fileType,parentNodeData){
    return function (dispatch) {
        ...
        const postData = {path:encodeURI(parentNodeData.fullPath+"/"+fileName),type:fileType};
        $.ajax({
            url,
            data:postData,
            type:'POST',
    ...
    return function (dispatch) {
        ...
        $.ajax({
            ...
            success:function (data) {
                ...
                dispatch({data:promoteTreeRoot(rootFile),type:LOAD_END});
...
export function createNewProject(newProjectName,storageType,parentNodeData) {
    return function (dispatch) {
        const url=window._server+'/frame/createProject';
        $.ajax({
            url,
            type:'POST',
            data:{newProjectName:newProjectName,storageType:storageType},
...
function buildData(data,level) {
    ...
    switch (data.type){
        ...
        case "rea":
            ...
            data._style=Styles.frameStyle.getRuleIconStyle();
            data.contextMenu=buildFileContextMenu();
            data.editorPath="/reaeditor";
            break;
        case "project":
            data._icon = data.storageType === 'db' ? 'rf rf-database' : Styles.frameStyle.getProjectIcon();
        ...
        case "ruleLib":
            ...
            data.contextMenu=[
                ...
                },
                {
                    ...
                    icon:Styles.frameStyle.getRuleIcon(),
                    click:function () {
                        event.eventEmitter.emit(event.OPEN_CREATE_FILE_DIALOG,{fileType:'rea.xml',nodeData:data})
                    }
                }
            ];
            break;
