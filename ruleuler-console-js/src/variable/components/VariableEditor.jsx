...
class VariableEditor extends Component{
    ...
    render(){
        ...
        const isGeneralEntity = masterRowData && masterRowData.type === 'GeneralEntity';
        const masterGridHeaders=[
            {id:'master-name',name:'name',label:'名称',filterable:true,editable:true,width:'130px'},
            {id:'master-type',name:'type',label:'类型',width:'120px',editorType:'select',selectData:['Custom','Clazz','GeneralEntity'],editable:true},
            {id:'master-clazz',name:'clazz',label:isGeneralEntity ? 'targetClass' : '类路径',filterable:true,editable:true}
        ];
        const slaveGridHeaders=[
            {id:'slave-name',name:'name',label:'字段名',filterable:true,editable:true},
            ...(isGeneralEntity ? [] : [{id:'slave-label',name:'label',label:'标题',filterable:true,width:'220px',editable:true}]),
        ...
        return (
            <div style={{margin:'0px'}}>
                <ReferenceDialog/>
                <Splitter orientation='vertical' position='50%' limit='450'>
