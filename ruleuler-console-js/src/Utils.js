...
export function getResourcePrefix() {
    const st = window._storageType;
    if (st === 'db') return 'dbr:';
    if (st === 'jcr') return 'jcr:';
    throw new Error('window._storageType is not set or invalid: ' + st);
}

export function stripResourcePrefix(path) {
    if (path.startsWith('jcr:')) return path.substring(4);
    if (path.startsWith('dbr:')) return path.substring(4);
    return path;
}
