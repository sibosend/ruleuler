...
/**
 * JCR 存储委托实现。
 * 继承 BaseRepositoryService，实现 RepositoryDelegate 接口。
 * 将原 RepositoryServiceImpl 中的所有 JCR 逻辑原样搬入。
 */
@Component("jcrRepositoryDelegate")
public class JcrRepositoryDelegate extends BaseRepositoryService implements RepositoryDelegate {
	...
	private RepositoryFile buildProjectFile(Node projectNode, FileType[] types, boolean classify, String searchFileName)
			throws Exception {
		...
		if ((types == null || types.length == 0)
				&& permissionService.projectPackageHasReadPermission(projectNode.getPath())) {
	...
	private void buildResources(Node projectNode, RepositoryFile libDir, FileType[] types, String searchFileName)
			throws Exception {
	...
	private void createResourceCategory(Node projectNode, RepositoryFile libDir, FileType[] types,
			String searchFileName) throws Exception {
		...
		if (types == null || types.length == 0) {
			librarySubTypes = new FileType[] { FileType.VariableLibrary, FileType.ParameterLibrary,
		...
		buildNodes(projectNode.getNodes(), decisionTableLib, libraryDecisionTypes, Type.decisionTableLib,
				searchFileName);
		buildNodes(projectNode.getNodes(), decisionTreeLib, libraryDecisionTreeTypes, Type.decisionTreeLib,
				searchFileName);
	...
	private void buildNodes(NodeIterator nodeIterator, RepositoryFile parent, FileType[] types, Type folderType,
			String searchFileName) throws Exception {
		...
		while (nodeIterator.hasNext()) {
			...
			if (name.toLowerCase().indexOf(RES_PACKGE_FILE) > -1 || name.toLowerCase().indexOf(CLIENT_CONFIG_FILE) > -1
					|| name.toLowerCase().indexOf(RESOURCE_SECURITY_CONFIG_FILE) > -1) {
				continue;
			}
			if (!fileNode.hasProperty(DIR_TAG)) {
				...
				if (libType.equals(LibType.res)) {
					if (!fileType.equals(FileType.ActionLibrary) && !fileType.equals(FileType.ParameterLibrary)
							&& !fileType.equals(FileType.ConstantLibrary)
							&& !fileType.equals(FileType.VariableLibrary)) {
						continue;
					}
				}
	...
	private void createSecurityConfigFile(User user) throws Exception {
		...
		Node rootNode = getRootNode();
		if (!rootNode.hasNode(filePath)) {
			createFileNode(filePath, "<?xml version=\"1.0\" encoding=\"utf-8\"?><user-permission></user-permission>",
					user, false);
		}
	}
	...
	public void setApplicationContext(ApplicationContext applicationContext) throws BeansException {
		this.repositoryBuilder = applicationContext.getBean("urule.repositoryBuilder", RepositoryBuilder.class);
