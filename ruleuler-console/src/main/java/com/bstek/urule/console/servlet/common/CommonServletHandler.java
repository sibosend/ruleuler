...
public class CommonServletHandler extends RenderPageServletHandler{
	...
	public void loadResourceTreeData(HttpServletRequest req, HttpServletResponse resp) throws Exception {
		...
		if(StringUtils.isNotBlank(forLib) && forLib.equals("true")){
			...
			types=new FileType[]{FileType.UL,FileType.Ruleset,FileType.RuleFlow,FileType.DecisionTable,FileType.ScriptDecisionTable,FileType.DecisionTree,FileType.Scorecard,FileType.Rea};
		}
		try{
			...
			if(files.startsWith("builtinactions")){
				...
				for(String path:paths){
					if (path.startsWith(RepositoryResourceProvider.JCR)) {
						path = path.substring(RepositoryResourceProvider.JCR.length());
					} else if (path.startsWith(DatabaseResourceProvider.DBR)) {
						path = path.substring(DatabaseResourceProvider.DBR.length());
					}
