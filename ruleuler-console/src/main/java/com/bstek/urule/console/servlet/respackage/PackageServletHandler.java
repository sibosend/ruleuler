...
public class PackageServletHandler extends RenderPageServletHandler {
	...
	private static Logger log = LoggerFactory.getLogger(PackageServletHandler.class);
	...
	public void pushKnowledgePackageToClients(HttpServletRequest req, HttpServletResponse resp) throws Exception {
		...
		logRequestDetails(req, "Request snapshot #1");
		...
		int byteSize = encodedContent.getBytes(StandardCharsets.UTF_8).length;
		double kbSize = byteSize / 1024.0;

		sb.append("知识包:").append(packageId)
		  .append(" 大小:").append(String.format("%.2f KB", kbSize)).append("<br>");
	...
	private boolean pushKnowledgePackage(String packageId,String content,String client){
		...
		try{
			...
			// log.info(
			// 	"Push knowledge package to client: packageId={}, clientUrl={}, content={}",
			// 	packageId, clientUrl, content
			// );
