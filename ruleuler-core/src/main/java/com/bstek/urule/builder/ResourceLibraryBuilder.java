...
public class ResourceLibraryBuilder extends AbstractBuilder{
	...
	public ResourceLibrary buildResourceLibrary(Collection<Library> libraries){
		...
		if(parameterVariableCategories.size()>0){
			VariableCategory merged=new VariableCategory();
			merged.setName(VariableCategory.PARAM_CATEGORY);
			merged.setClazz(HashMap.class.getName());
			merged.setType(CategoryType.Clazz);
			Map<String,Variable> seen=new LinkedHashMap<>();
			for(VariableCategory vc:parameterVariableCategories){
				if(vc.getVariables()==null){
					continue;
				}
				for(Variable v:vc.getVariables()){
					Variable existing=seen.get(v.getName());
					if(existing!=null && !existing.getType().equals(v.getType())){
						throw new RuleException("Parameter ["+v.getName()+"] has conflicting types: "+existing.getType()+" vs "+v.getType());
					}
					seen.put(v.getName(),v);
				}
			}
			merged.setVariables(new ArrayList<>(seen.values()));
			VariableLibrary parameterLib=new VariableLibrary();
			parameterLib.addVariableCategory(merged);
