...
public class Criteria extends BaseCriterion implements BaseCriteria{
	...
	public EvaluateResponse evaluate(EvaluationContext context,Object obj,List<Object> allMatchedObjects){
		...
		boolean result;
		try {
			result=context.getAssertorEvaluator().evaluate(leftResult, right, datatype,op);
		} catch (Exception e) {
			String fieldName = (leftPart instanceof VariableLeftPart) ? ((VariableLeftPart)leftPart).getVariableName() : "unknown";
			throw new RuleException("Field [" + fieldName + "] comparison failed, left=" + leftResult + ", right=" + right + ", datatype=" + datatype + ", op=" + op + ", cause: " + e.getMessage());
		}
